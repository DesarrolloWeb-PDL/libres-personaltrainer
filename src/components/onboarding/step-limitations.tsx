...
export function StepLimitations({ data, onUpdate }: StepLimitationsProps) {
  return (
    <div className="space-y-6">
      ...
      <div>
        <label htmlFor="injuries" className="block text-sm font-medium text-zinc-300">
          Injuries or Limitations <span className="text-zinc-500">(optional)</span>
        </label>
        <textarea
          id="injuries"
          rows={4}
          value={data.injuries ?? ""}
          onChange={(e) => onUpdate({ injuries: e.target.value || undefined })}
      ...
      <div className="rounded-xl bg-lime-500/10 border border-lime-500/20 p-4">
        <p className="text-sm text-lime-400">
          <strong>Why we ask:</strong> This information helps the training engine suggest safer
          exercise substitutions and avoid movements that might aggravate existing conditions.
        </p>
      </div>
    </div>
  );
