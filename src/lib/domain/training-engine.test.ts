...
const exercisePool: Exercise[] = [
  ...
  makeExercise({ id: 'lateral-raise', name: 'Lateral Raise', muscleGroup: 'shoulders', isCompound: false }),
  makeExercise({ id: 'tricep-pushdown', name: 'Tricep Pushdown', muscleGroup: 'triceps', isCompound: false }),
  ...
  makeExercise({ id: 'romanian-dl', name: 'Romanian Deadlift', muscleGroup: 'hamstrings', isCompound: true }),
  makeExercise({ id: 'glute-bridge', name: 'Glute Bridge', muscleGroup: 'glutes', isCompound: false }),
...
describe('selectSplit', () => {
  ...
  it('returns upper_lower for 4 days/week (any experience)', () => {
    expect(selectSplit(makeProfile({ trainingFrequency: 4, experienceLevel: 'beginner' }))).toBe('upper_lower')
    expect(selectSplit(makeProfile({ trainingFrequency: 4, experienceLevel: 'intermediate' }))).toBe('upper_lower')
    expect(selectSplit(makeProfile({ trainingFrequency: 4, experienceLevel: 'advanced' }))).toBe('upper_lower')
  })

  it('returns push_pull_legs for 5+ days with advanced experience', () => {
    expect(selectSplit(makeProfile({ trainingFrequency: 5, experienceLevel: 'advanced' }))).toBe('push_pull_legs')
    expect(selectSplit(makeProfile({ trainingFrequency: 6, experienceLevel: 'advanced' }))).toBe('push_pull_legs')
  })

  it('returns upper_lower for 5+ days with intermediate experience', () => {
    expect(selectSplit(makeProfile({ trainingFrequency: 5, experienceLevel: 'intermediate' }))).toBe('upper_lower')
    expect(selectSplit(makeProfile({ trainingFrequency: 6, experienceLevel: 'intermediate' }))).toBe('upper_lower')
  })

  it('returns upper_lower for 5+ days with beginner experience', () => {
    expect(selectSplit(makeProfile({ trainingFrequency: 5, experienceLevel: 'beginner' }))).toBe('upper_lower')
  })

  it('returns full_body for 7 days (max frequency, non-advanced)', () => {
    expect(selectSplit(makeProfile({ trainingFrequency: 7, experienceLevel: 'beginner' }))).toBe('upper_lower')
  })

  it('returns push_pull_legs for 7 days with advanced', () => {
    expect(selectSplit(makeProfile({ trainingFrequency: 7, experienceLevel: 'advanced' }))).toBe('push_pull_legs')
  })
})
...
describe('generateProgram', () => {
  it('generates a program with correct number of days', () => {
    const program = generateProgram(
      makeProfile({ trainingFrequency: 4 }),
      exercisePool,
    )
  ...
  it('full_body split generates full body day names', () => {
    const program = generateProgram(
      makeProfile({ trainingFrequency: 3 }),
      exercisePool,
    )
    for (const day of program.days) {
